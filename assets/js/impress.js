(function (document, window) {
    ...
    var impressSupported = (pfx("perspective") !== null) && (body.classList) && (body.dataset) && (ua.search(/(iphone)|(ipod)|(android)/) === -1);
    if (!impressSupported) {
        ...
        document.addEventListener("keyup", function (event) {
            if (event.keyCode === 9 || (event.keyCode >= 32 && event.keyCode <= 34) || (event.keyCode >= 37 && event.keyCode <= 40)) {
                switch (event.keyCode) {
                    ...
                    case 38:
                        api.prev();
                        // piwikTracker.trackPageView(window.location.hash);
                        break;
                    case 9:
                    case 32:
                    case 34:
        ...
        document.addEventListener("touchstart", function (event) {
            if (event.touches.length === 1) {
                ...
                if (x < width && y > height) {
                    result = api.prev();
                    // piwikTracker.trackPageView(window.location.hash);
                } else if (x > window.innerWidth - width && y > height) {
                    result = api.next();
                    // piwikTracker.trackPageView(window.location.hash);
                }
                if (result) {
